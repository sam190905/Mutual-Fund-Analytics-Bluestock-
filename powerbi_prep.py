...
fm.to_csv(f"{PBI_DIR}/dim_fund.csv", index=False)
...
nav.to_csv(f"{PBI_DIR}/fact_nav.csv", index=False)
...
aum.to_csv(f"{PBI_DIR}/fact_aum.csv", index=False)
...
sip.to_csv(f"{PBI_DIR}/fact_sip_industry.csv", index=False)
...
cat.to_csv(f"{PBI_DIR}/fact_category_inflows.csv", index=False)
...
folio.to_csv(f"{PBI_DIR}/fact_folio_count.csv", index=False)
...
perf.to_csv(f"{PBI_DIR}/fact_performance.csv", index=False)
...
tx.to_csv(f"{PBI_DIR}/fact_transactions.csv", index=False)
print(f"✓ fact_transactions.csv {tx.shape}")

# ── 9. fact_portfolio ─────────────────────────────────────────
port = pd.read_csv(f"{RAW}/09_portfolio_holdings.csv")
port.to_csv(f"{PBI_DIR}/fact_portfolio.csv", index=False)
...
bench.to_csv(f"{PBI_DIR}/fact_benchmark.csv", index=False)
...
dim_date.to_csv(f"{PBI_DIR}/dim_date.csv", index=False)
...
kpi.to_csv(f"{PBI_DIR}/kpi_summary.csv", index=False)
print(f"✓ kpi_summary.csv       {kpi.shape}")

print(f"\n✓ All Power BI files saved → data/powerbi/")
print(f"  Total files: {len(os.listdir(PBI_DIR))}")
