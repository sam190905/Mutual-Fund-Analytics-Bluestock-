import pandas as pd
from paths import RAW_DIR, REPORTS_DIR

# 01_fund_master.csv — provided dataset (real schema with amfi_code column)
fund_master = pd.read_csv(RAW_DIR / "01_fund_master.csv")

# 02_nav_history.csv — provided dataset (real schema with amfi_code column)
...
print(f"Fund Master records    : {len(fund_master)} schemes")
print(f"NAV History records    : {len(nav_history)} rows")
...
with open(REPORTS_DIR / "data_quality_day1.txt", "w") as f:
