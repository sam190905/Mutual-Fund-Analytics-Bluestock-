...
import pandas as pd
import numpy as np
from paths import PROCESSED_DIR, RAW_DIR, display_path
...
nav_clean.to_csv(PROCESSED_DIR / "clean_02_nav_history.csv", index=False)
...
tx.to_csv(PROCESSED_DIR / "clean_08_investor_transactions.csv", index=False)
...
perf.to_csv(PROCESSED_DIR / "clean_07_scheme_performance.csv", index=False)
...
for fname in others:
    ...
    if src.exists():
        df = pd.read_csv(src)
        out = PROCESSED_DIR / f"clean_{fname}"
        df.to_csv(out, index=False)
        print(f"   {fname:<35} {str(df.shape)}")
    else:
        print(f"   Not found: {display_path(src)}")
